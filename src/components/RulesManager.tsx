...
const EMOTIONS = ['Calm / Confident', 'Excited / Rushed', 'Fearful / Hesitant', 'Revenge', 'Neutral'];

export const RulesManager = ({ isOpen, onClose, showToast }: { isOpen: boolean, onClose: () => void, showToast: (msg: string, type?: 'success' | 'error') => void }) => {
    ...
    if (isOpen && auth.currentUser) {
      const fetchRules = async () => {
        try {
          ...
          if (docSnap.exists()) {
            setRules(docSnap.data() as Rules);
          }
        } catch (e) {
            console.error(e);
        } finally {
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                        ...
                        <div className="flex gap-2">
                            <input type="number" value={rules.maxDailyLoss} onChange={e => setRules({...rules, maxDailyLoss: Number(e.target.value)})} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm w-full outline-none" />
                   ...
                   <div>
                        ...
                        <input type="number" value={rules.maxTradesPerDay} onChange={e => setRules({...rules, maxTradesPerDay: Number(e.target.value)})} className="bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm w-full outline-none" />
                   </div>
                </div>
